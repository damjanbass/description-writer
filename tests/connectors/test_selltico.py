"""Tests for the Selltico connector placeholder.

`SellticoConnector` is not a working integration (see its module docstring):
no public Selltico API documentation exists, so every method raises
`NotImplementedError` rather than guessing at an API shape. These tests cover
exactly that contract — Protocol conformance, constructor storage, and that
both methods raise with a message naming "Selltico" — and nothing else, since
there is no behaviour beyond that to exercise.
"""
...
def _make_connector(*, transport: object | None = None) -> SellticoConnector:
    return SellticoConnector(
...
class TestFetchProductsRaises:
    def test_raises_not_implemented_error_naming_platform(self):
        connector = _make_connector()
        with pytest.raises(NotImplementedError, match="Selltico"):
